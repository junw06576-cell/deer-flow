...
For an Agent that needs a hard capability boundary, an operator can configure a non-empty `tool_names` allowlist. It is applied after configured, built-in, MCP, and deferred tools are assembled, before any schema reaches the model; omitted tools cannot be discovered or executed. Such an Agent can also declare public `mandatory_skills`, whose validated instructions and `allowed-tools` policy are active before the first model call. These fields are operator-authored configuration, not self-service Agent-editing controls.
