...
需求命中高风险早筛（①②⑤⑥），或描述含结算、费用、医保、上传、校验、报表、接口、明细、账单等可映射业务概念时运行；**且仅在事前质控尚未给出 `NEED-INFO`/`NEED-REVIEW` 阻断终局时运行**——纯文本可定的终局（软件质量、PIMIS A 级、时效不可处理、§6a 治标警惕等）已锁定终局时跳过整个发现链（见 `../config/qc-rules.md §6a` 与 SKILL.md 步骤 3「终局早退」）。纯文字、无业务实体且非高风险的需求主动跳过，记 `kb.note=未触及判定面，跳过`；这不是降级。
