...
分析时按命中类别的必需维度（见 `analysis-description-writing-rules.md`）扫描出的"未说清"项，先按下表分流，再套本启发式——**呈现类**缺口用合理默认兜进变更方案 §七"假设"、不进 `analysis_gaps`、**不计入**下表"不满足"；只有**正确性类**缺口缺失才按"不满足"处理。
