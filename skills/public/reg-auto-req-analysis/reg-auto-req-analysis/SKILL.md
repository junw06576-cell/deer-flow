...
所有手工产物只能放在 `过程文件/<id>/<run_id>/`；来源附件放 `附件/`，解析副本放 `附件解析/`。执行器审计放 `过程文件/<id>/runs/`；跨运行测试记录只追加到 `过程文件/测试日志.md`。重分析诊断（仅重跑 + 有补充时）追加到 `过程文件/skill-feedback.md`，非 TFS、不入计划 JSON、不进执行器审计。
